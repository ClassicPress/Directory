...
function display_author_plugins()
{
	...
	if ($custom_posts->have_posts()) {
		...
		while ($custom_posts->have_posts()) {
			...
			$content .= '<li>';
			$content .= sprintf('<a href="%s" title="%s plugin">%s</a>', esc_url(get_permalink(), esc_attr(get_the_title()),esc_html($author_display_name)));
			$content .= '</li>';
		}
...
function display_author_themes()
{
	...
	if ($custom_posts->have_posts()) {
		...
		while ($custom_posts->have_posts()) {
			$custom_posts->the_post();
			$content .= '<li>';
			$content .= sprintf('<a href="%s" title="%s theme">%s</a>', esc_url(get_permalink(), esc_attr(get_the_title()),esc_html($author_display_name)));
			$content .= '</li>';
		}
