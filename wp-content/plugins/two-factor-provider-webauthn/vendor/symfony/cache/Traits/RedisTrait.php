...
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Command\Redis\UNLINK;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Connection\Aggregate\ClusterInterface;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Connection\Aggregate\RedisCluster;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Connection\Aggregate\ReplicationInterface;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Response\ErrorInterface;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Response\Status;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Symfony\Component\Cache\Exception\CacheException;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException;
...
trait RedisTrait
{
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if (preg_match('#[^-+_.A-Za-z0-9]#', $namespace, $match)) {
            throw new InvalidArgumentException(sprintf('RedisAdapter namespace WildWolf\WordPress\TwoFactorWebAuthn\Vendor\contains "%s" but only characters in [-+_.A-Za-z0-9] are allowed.', $match[0]));
        }
    ...
    public static function createConnection(string $dsn, array $options = [])
    {
        ...
        if (null === $params['class'] && \extension_loaded('redis')) {
            ...
            if (isset($params['redis_sentinel']) && !is_a($class, \Predis\Client::class, true) && !class_exists(\RedisSentinel::class)) {
                throw new CacheException(sprintf('Cannot use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Redis Sentinel: class "%s" does not extend "Predis\Client" and ext-redis >= 5.2 not found: "%s".', $class, $dsn));
            }
        }
