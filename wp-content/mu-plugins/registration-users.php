...
/** ADD SIGN UP DATE COLUMN TO USER LIST AND MAKE IT SORTABLE */
// Add the custom column to the admin screen
add_filter('manage_users_columns', 'add_user_signup_date_column');
function add_user_signup_date_column($columns)
{
	$columns['signup_date'] = 'Sign Up Date';
	return $columns;
}

// Populate the new column with the signup date
add_action('manage_users_custom_column',  'show_user_signup_date_column_content', 10, 3);
function show_user_signup_date_column_content($value, $column_name, $user_id)
{
	if ('signup_date' == $column_name) {
		$user = get_userdata($user_id);
...
add_filter('manage_users_sortable_columns', 'make_signup_date_column_sortable');
function make_signup_date_column_sortable($columns)
{
	$columns['signup_date'] = 'user_registered';
	return $columns;
}
