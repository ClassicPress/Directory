...
function kts_render_md($md)
{
	...
	if (isset($response['response']['code']) && $response['response']['code'] === 200) {
		...
		// Delete everything after $end_marker
		$end_marker = '<!-- README END -->';
		$end_marker_position = strpos($response['body'], $end_marker);

		if ($end_marker_position !== false) {
			$response['body'] = substr($response['body'], 0, $end_marker_position + strlen($end_marker));
		}
